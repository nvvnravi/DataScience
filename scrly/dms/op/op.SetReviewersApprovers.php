	...
	if (is_numeric($p)) {
		if (isset($accessIndex["i"][$p])) {
			...
			//if (!isset($approvalIndex["i"][$p])) {
				...
				error_log("innoright  - Here*****************= 31 ".$res, 0);
				$res = ($res > 0 ? 0 : $res);
				switch ($res) {
					case 0:
...
if (count($approvalIndex["g"]) > 0) {
	//error_log(" innoright  -   Here************** = 13.", 0);
	foreach ($approvalIndex["g"] as $rx=>$rv) {
		//error_log(" innoright  -   Here************** = 14.", 0);
		if ($rv["status"] == 0) {
			//error_log(" innoright  -   Here************** = 15.", 0);
			// User is to be removed from the approvers list.
			if (!isset($docAccess["groups"][$accessIndex["g"][$rx]])) {
				//error_log(" innoright  -   Here************** = 16.", 0);
...
*/
				...
				//error_log(" innoright  -   Here************** = 17  res value is : ".$res, 0);
				switch ($res) {
					case 0:
