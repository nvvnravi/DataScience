	...
	function getStatisticalData($type='') { /* {{{ */
		...
		switch($type) {
			...
			case 'docsperstatus':
				...
				return $resArr;
				case 'dsl':
				// TODO: Need to  revisit this query.
				//this query gives latest status of all documents including just 
				// version (not all version statuses)
				$queryStr="select a.`id` as `docid`, a.name as `docname` , c.status as `key`,b.version as `version`,max(c.`statusLogID`) as maxlog  from `tblDocuments` a left join `tblDocumentStatus` b on a.id=b.`documentID` left join `tblDocumentStatusLog` c on b.`statusID`=c.`statusID` group by a.`id`  order by a.id";
				
				$queryStr = "SELECT b.documentID as `docid`,c.name as `docname`,max(b.`version`) as `version`,a.statusID as `statusid`,max(a.`status`) as `key` , c.`folderList` as `docpath` FROM `tbldocumentstatuslog` a,`tbldocumentstatus` b,`tbldocuments` c where c.id=b.documentID and a.statusID=b.statusID group by c.id order by a.statusID";
				
				$queryStr="select b.`status` as `key`,a.`id` as `docid`,a.`name` as `docname`,a.`version`,b.statusID as `statusid`,a.`folderList` as `docpath` from (select a.`id`,a.`name`,max(b.`version`) as `version`, max(c.`statusLogID`) as maxlog,a.`folderList` from `tblDocuments` a left join `tblDocumentStatus` b on a.id=b.`documentID` left join `tblDocumentStatusLog` c on b.`statusID`=c.`statusID` where a.id=b.documentID group by a.`id` order by a.id) a left join `tblDocumentStatusLog` b on a.maxlog=b.`statusLogID` order by b.`status`";
				...
				return $resArr;
			case 'dslRejected':
				$queryStr="select b.`status` as `key`,a.`id` as `docid`,a.`name` as `docname`,a.`version`,b.statusID as `statusid`,a.`folderList` as `docpath` from (select a.`id`,a.`name`,max(b.`version`) as `version`, max(c.`statusLogID`) as maxlog,a.`folderList` from `tblDocuments` a left join `tblDocumentStatus` b on a.id=b.`documentID` left join `tblDocumentStatusLog` c on b.`statusID`=c.`statusID` where a.id=b.documentID group by a.`id` order by a.id) a left join `tblDocumentStatusLog` b on a.maxlog=b.`statusLogID` where b.`status`= -1 order by b.`status`";
