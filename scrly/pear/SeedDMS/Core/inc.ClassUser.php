...
class SeedDMS_Core_User { /* {{{ */
	...
	/**
	 * Checks if user is member of a given group By GroupName
	 *
	 * @param SeedDMS_Core_Group $groupName
	 * @return boolean true if user is member of the given group otherwise false
	 */
	function doesUserBelongToGroup($groupName) { /* {{{ */
	//error_log("In doesUserBelongToGroup - ".$groupName,0);
		foreach($this->getGroups() as $k){
			//error_log("In looping group names - ".$k->getName(),0);
		if( strcmp($k->getName(), $groupName) == 0){
			return true;
		}
	...
	function getApprovalStatus($documentID=null, $version=null) { /* {{{ */
		...
		$queryStr =
			...
			"ORDER BY  `tblDocumentApprovers`.`approveID` desc, `tblDocumentApproveLog`.`approveLogID` DESC";
