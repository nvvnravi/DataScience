...
class SeedDMS_Core_Group { /* {{{ */
	...
	function getApprovalStatus($documentID=null, $version=null) { /* {{{ */
		...
		$queryStr = "SELECT `tblDocumentApprovers`.*, `tblDocumentApproveLog`.`status`, ".
			...
			"AND `tblDocumentApprovers`.`required`='". $this->_id ."' ORDER BY `tblDocumentApprovers`.`approveID` desc";
		$resArr = $db->getResultArray($queryStr);
		if (is_bool($resArr) && $resArr == false)
			return false;
		if (count($resArr)>0) {
			foreach ($resArr as $res){
			error_log("innoright - inc.classgroup.php getApprovalStatus - ".$res["approveID"]."   ".$res["status"],0);
				$status[] = $res;
			}
		}
