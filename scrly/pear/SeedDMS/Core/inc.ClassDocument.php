...
class SeedDMS_Core_DocumentContent extends SeedDMS_Core_Object { /* {{{ */
	...
	function addIndApprover($user, $requestUser) { /* {{{ */
		...
		$userID = $user->getID();
error_log("Here userID :".$userID, 0);
		// Get the list of users and groups with read access to this document.
		if($this->_document->getAccessMode($user) < M_READ) {
			return -2;
		}
		...
		if (is_bool($approvalStatus) && !$approvalStatus) {
			error_log("Here approvalStatus :".$approvalStatus, 0);
			return -1;
		}
		$indstatus = false;
		if (count($approvalStatus["indstatus"]) > 0) {
			...
			if($indstatus["status"]!=-2) {
				...
				//return -3; // commented by Innoright
			}
		}
		...
		if ( !$indstatus || (isset($indstatus["status"]) && $indstatus["status"]!=-2)) {
			...
			$res = $db->getResult($queryStr);
			if (is_bool($res) && !$res) {
				error_log("Here tblDocumentApprovers :");
				return -1;
			}
		...
		$res = $db->getResult($queryStr);
		if (is_bool($res) && !$res) {
			error_log("Here tblDocumentAppriveLog");
			return -1;
		}
	...
	function addGrpApprover($group, $requestUser) { /* {{{ */
		...
		//Innoright if (count($approvalStatus) > 0 && $approvalStatus[0]["status"]!=-2) {
		...
		if (!isset($approvalStatus[0]["status"]) || (isset($approvalStatus[0]["status"]) && $approvalStatus[0]["status"]!=-2)) {
			$queryStr = "INSERT INTO `tblDocumentApprovers` (`documentID`, `version`, `type`, `required`) ".
				"VALUES ('". $this->_document->getID() ."', '". $this->_version ."', '1', '". $groupID ."')";
			$res = $db->getResult($queryStr);
			//error_log(" innoright  Here************** tblDocumentApprovers=".$res, 0);
			if (is_bool($res) && !$res) {
				//error_log("innoright   Here************** tblDocumentApprovers", 0);
				//error_log("innoright   Here************** tblDocumentApprovers", 0);
				return -1;
			}
			$approveID = $db->getInsertID('tblDocumentApprovers', 'approveID');
		...
		$res = $db->getResult($queryStr);
		if (is_bool($res) && !$res) {
			//error_log("Here************** tblDocumentApproveLog", 0);
			return -1;
		}
	...
	function setApprovalByInd($user, $requestUser, $status, $comment, $file='') { /* {{{ */
		...
		error_log("innoright  - setApprovalByInd  ", 0);
		...
		if ($indstatus["status"] == $status)
			return 0;
		error_log("  approve id  ".$indstatus["approveID"],0);
		error_log(" user id   ".$requestUser->getID(),0);
	...
	function setApprovalByGrp($group, $requestUser, $status, $comment, $file='') { /* {{{ */
		$db = $this->_document->_dms->getDB();
	//error_log("innoright  - setApprovalByGrp  ===  ".$status."    ".$comment, 0);
		// Check to see if the user can be removed from the approval list.
		$approvalStatus = $group->getApprovalStatus($this->_document->getID(), $this->_version);
		//error_log("innoright  - setApprovalByGrp  ",0);
		if (is_bool($approvalStatus) && !$approvalStatus) {
			//error_log("innoright  - one  ",0);
			return -1;
		}
		if (count($approvalStatus)==0) {
			//error_log("innoright  - two  ",0);
			// User is not assigned to approve this document. No action required.
			// Return an error.
			return -3;
		}
		if ($approvalStatus[0]["status"]==-2) {
			error_log("innoright  - three  ",0);
		...
		//error_log("innoright approvalStatus length : ".count($approvalStatus),0);
		
		//error_log("status: ".$status,0);
		if ($approvalStatus[0]["status"] == $status){
			error_log("innoright  - four  ",0);
			return 0;
		}
		
//error_log("  approve id  ".$approvalStatus[0]["approveID"],0);
//error_log(" user id   ".$requestUser->getID(),0);
	...
	function delGrpApprover($group, $requestUser, $msg='') { /* {{{ */
		...
		$approvalStatus = $group->getApprovalStatus($this->_document->getID(), $this->_version);
		if (is_bool($approvalStatus) && !$approvalStatus) {
			error_log("In delGrpApprover - Innoright : Check to see if the user can be removed from the approver list",0);
			return -1;
		}
		if (count($approvalStatus)==0) {
			// User is not assigned to approve this document. No action required.
			// Return an error.
			error_log("In delGrpApprover - Innoright : User is not assigned to approve",0);
			return -2;
		}
		if ($approvalStatus[0]["status"]!=0) {
			// User has already submitted an approval or has already been deleted;
			// return an error.
			error_log("In delGrpApprover - Innoright : User has alredy submitted an approval",0);
			return -3;
		}
